...
class MySQLRecover extends BaseAbstract
{
    public const CMD = 'zcat %s | /usr/bin/mysql -u %s -p%s -h 127.0.0.1 -P %s %s';

    public function configure()
    {
        $this->setName('mjrone:backup:recover')
            ->setHelp('recover Backup from Database Server')
            ->setDescription('recover Backup from MySQL Server and Store it in the Vagrant Backup Directory');
    }
            ...
            foreach($databases as $id=>$database)
            {
                ...
                if($database->getType()==='mysql')
                {
                    try{
                        ...
                        $cmd = sprintf(self::CMD, $file, $dbuser, $password, $port, $database);
                        $outputLvl = 3;
